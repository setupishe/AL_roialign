...
for range in "${ranges[@]}"; do
  ...
  python3 prepare_al_split.py \
    --weights /home/setupishe/ultralytics/runs/detect/VOC_${folder_name}_${range}_s_matryoshka_iter_correct_ctf/weights/best.pt \
    --from-fraction $range \
  ...
  batch=48 \
  name=VOC_distance_0${next_range}_s_matryoshka_iter_correct_ctf \
  device=$device \
  epochs=65 \
  matryoshka=True
done