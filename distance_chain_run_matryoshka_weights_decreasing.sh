...
for range in "${ranges[@]}"; do
  ...
  python3 prepare_al_split.py \
    --weights /home/setupishe/ultralytics/runs/detect/VOC_${folder_name}_${range}_s_matryoshka_weights_decreasing/weights/best.pt \
    --from-fraction $range \
  ...
  batch=48 \
  name=VOC_distance_0${next_range}_s_matryoshka_weights_decreasing \
  device=$device \
  epochs=65 \
  matryoshka=True \
  matryoshka_weights="1.0, 0.8, 0.4, 0.2"
done